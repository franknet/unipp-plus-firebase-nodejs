...
const utils = require("../../../utils/number-utils");

exports.buildGrades = (nf, me) => {
...
function setStatus(discipline) {
  ...
  const np1Num = utils.toNumber(np1);
  const np2Num = utils.toNumber(np2);
  const msNum = utils.toNumber(ms);
  const exNum = utils.toNumber(ex);
  const mfNum = utils.toNumber(mf);
  ...
  if (utils.isNumber(np1) && !utils.isNumber(np2)) {
  ...
  if (utils.isNumber(np2) && utils.isNumber(ms) && !utils.isNumber(ex)) {
    const minGrade = 10 - msNum;
    status.message.push(`Você precisa tirar no mínimo ${_.round(minGrade, 1)} no exame`);
  }
  if (utils.isNumber(np1) && utils.isNumber(np2) && utils.isNumber(mf)) {
